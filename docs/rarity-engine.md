...
**stolen_base_burst requires a corpus rebuild to surface.** `corpus.py` now has a `_detect_stolen_base_burst` detector (a team stealing 4+ bases in a game), so the pattern is wired end to end. but stolen bases are baserunning events (`BAT_EVENT_FL == 'F'`), which the other detectors' batting-only filter drops — `_extract_patterns_from_events` therefore derives `batting_team`/`date` on the full event set and scans the unfiltered events for steals. the committed `retrosheet-patterns.parquet` predates the detector and still contains zero `stolen_base_burst` rows; rerun the **Update Retrosheet Corpus** workflow to regenerate it before this pattern can match. caveat: retrosheet logs a double/triple steal as one `EVENT_CD==4` row, so the detector counts steal events and slightly undercounts multi-runner steals relative to the box-score total the live side reads.
