...
**5-year gap threshold.** the tier 3 threshold of >1825 days before surfacing an event is a judgment call. "10+ Ks in a game" happens dozens of times per season so the corpus match is rarely interesting, while "walk-off grand slam" is interesting at much shorter gaps. a per-pattern threshold config would be an improvement.
